defmodule FluxonUIFirst.TodosTest do
  use FluxonUIFirst.DataCase
  ...
  describe "todos" do
    ...
    import FluxonUIFirst.AccountsFixtures, only: [user_scope_fixture: 0]
    import FluxonUIFirst.TodosFixtures
    ...
    test "create_todo/2 with valid data creates a todo" do
      valid_attrs = %{description: "some description", due: ~D[2025-10-14]}
      scope = user_scope_fixture()

      assert {:ok, %Todo{} = todo} = Todos.create_todo(scope, valid_attrs)
      assert todo.description == "some description"
      assert todo.due == ~D[2025-10-14]
    ...
    test "update_todo/3 with valid data updates the todo" do
      scope = user_scope_fixture()
      todo = todo_fixture(scope)
      update_attrs = %{description: "some updated description", due: ~D[2025-10-15]}

      assert {:ok, %Todo{} = todo} = Todos.update_todo(scope, todo, update_attrs)
      assert todo.description == "some updated description"
      assert todo.due == ~D[2025-10-15]
    end
