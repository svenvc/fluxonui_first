defmodule FluxonUIFirstWeb.UserAuthTest do
  use FluxonUIFirstWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias FluxonUIFirst.Accounts
  alias FluxonUIFirst.Accounts.Scope
  alias FluxonUIFirstWeb.UserAuth

  import FluxonUIFirst.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, FluxonUIFirstWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      FluxonUIFirstWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_authenticated" do
    ...
    test "redirects to login page if there isn't a valid user_token", %{conn: conn} do
      ...
      socket = %LiveView.Socket{
        endpoint: FluxonUIFirstWeb.Endpoint,
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        FluxonUIFirstWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
